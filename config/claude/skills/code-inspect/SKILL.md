...
Skill tool で `/comment-review` を実行し、今回の差分で追加・変更したコメントを監査する。
不要なコメントを残したままレビューに回すと、指摘が「本質的な問題」から逸れる。
...
Step 5 でユーザーが承認した修正を実施する。

修正完了後、**必ず Step 1 に戻り、Step 1 → Step 2 → Step 3 → Step 4 → Step 5 を実際に再実行する**。
